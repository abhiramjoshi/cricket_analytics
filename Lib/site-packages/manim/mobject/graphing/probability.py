...
class BarChart(Axes):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        x_length = x_length if x_length is not None else config.frame_width - 4
        ...
        if x_range is None:
            ...
            x_range = [*x_range, round(x_range[1] // x_length, 0)]
    ...
    def _add_x_axis_labels(self):
        ...
        for i, (value, bar_name) in enumerate(zip(val_range, self.bar_names)):
            ...
            bar_name_label.rotate(self.label_rotation)
    ...
    def _create_bar(self, bar_number: int, value: float) -> Rectangle:
        ...
        y_cutoff = min(abs(min_y), abs(max_y))
        if min_y < 0 and max_y > 0:
            y_cutoff = 0
        # distance from between the y-axis and the top of the bar
        if value < y_cutoff:
            value = y_cutoff
        bar_h = abs(self.c2p(0, value)[1] - self.c2p(0, y_cutoff)[1])
        # width of the bar
        ...
        bar.next_to(self.c2p(bar_number + 0.5, y_cutoff), pos, buff=0)
        return bar

    def _add_bars(self, x_range) -> None:
        for i, value in zip(np.arange(x_range[0], x_range[1]), self.values):
    ...
    def get_bar_labels(
        ...
        **kwargs
        ...
        if not isinstance(constr_args, list):
            constr_args = [constr_args.copy() for x in range(len(self.bars))]
        elif len(constr_args) > len(self.bars):
            raise Exception('Too many label definitions for number of bars in graph')

        if values:
            if len(values) != len(self.bars):
                if len(values) < len(self.bars):
                    values += ['']*(len(self.bars) - len(values))
            _values = values 
        else:
            _values = self.values

        bar_labels = VGroup()
        for bar, value, c_args in zip(self.bars, _values, constr_args):
            if c_args.pop('no_val', False):
                bar_lbl = label_constructor(**c_args)    
            else:
                bar_lbl = label_constructor(str(value), **c_args)
